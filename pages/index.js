...
function DateFormat(ap) {
   let date = new Date(ap)
  var monthNames = [
    ...
    return day + ' ' + monthNames[monthIndex] + ' ' + year + ' '+ time+":"+time2 ;
...
export default class News extends React.Component {
  ...
  ///Was Trying to include search feature but wasnt sure how to implement it!
  setNewsQ = (input) => {
    this.setState({
      q: input,
  ...
  render()  {
    ...
    return (
      <body>
        ...
        <div>
          {/* Unsure why select returns undefined newsSources*/}
        <SelectForm setNewsSource={this.setNewsSource} />
        ...
        <h3>{this.state.newsSource.split("-").join(" ")}</h3>
          ...
          {this.state.articles.map((article, index) => (
            <section key={index}>
              ...
              <p><Link as={`/a/id:${index}`} href={`/a?id:${index}`}><a>Read More</a></Link></p>
            </section>
          ))}
        </div>
